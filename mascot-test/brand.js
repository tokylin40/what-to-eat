(() => {
  ...
  function makeStage(kind, title, copy, asset='home') {
    ...
    el.innerHTML =
      '<img src="' + ASSETS[asset] + '" alt="Open Pengu 企鵝吉祥物">' +
  ...
  function decorate() {
    ...
    if (hero) {
      const stage = makeStage('home','Open Pengu 幫你決定 🍽️','你負責按，我負責把晚餐逼出來。','home');
    ...
    if (wheel) {
      ...
      const stage = makeStage('wheel', detail ? '再細一點，我來選' : '命運交給企鵝 🎡',
        detail ? '大方向決定了，再轉一次就收工。' : '不要算機率，直接按下去。','judge');
    ...
    if (text.includes('我的胃')) {
      const stage = makeStage('stomach','企鵝正在研究你的胃','玩越多次，我越知道你其實沒有「都可以」。','home');
    ...
    if (text.includes('三問淘汰賽') || text.includes('最後一輪')) {
      const stage = makeStage('tournament','企鵝裁判上線 🥊','照直覺按。這次規則不一定跟上一局一樣。','judge');
      const hint = panel.querySelector('.hint');
      if (hint) hint.after(stage); else panel.prepend(stage);
      return;
    }
    ...
    if (text.includes('地獄')) {
      const stage = makeStage('tournament','地獄企鵝不等人 🔥','三秒內回答；你不選，我就替你選。','judge');
      const hint = panel.querySelector('.hint');
      if (hint) hint.after(stage); else panel.prepend(stage);
      return;
    }
